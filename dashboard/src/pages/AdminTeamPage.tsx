...
function TeamManager() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.success(isEditMode ? "Team member updated." : "Team member added.");
      resetForm();
      await load();
    } catch {
  ...
  const handleDelete = async (member: TeamMemberRow) => {
    if (!window.confirm(`Delete ${member.name} from the team roster?`)) return;
    ...
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isEditMode
              ? `Updating ${editing.name}. Leave photo empty to keep the current image.`
              : "New members appear on the marketing homepage team section."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form className="grid gap-4" onSubmit={handleSubmit}>
            {isEditMode ? (
              <div className="flex items-center gap-3 rounded-xl border bg-muted/30 p-3">
                <Avatar className="size-12">
                  {mediaUrl(editing.photoUrl) ? (
                    <AvatarImage
                      src={mediaUrl(editing.photoUrl)}
                      alt={editing.name}
                    />
                  ) : null}
                  <AvatarFallback>{initials(editing.name)}</AvatarFallback>
                </Avatar>
                <div className="min-w-0 text-xs text-muted-foreground">
                  <p className="font-medium text-foreground">Current photo</p>
                  <p className="truncate">{editing.photoUrl}</p>
                </div>
              </div>
            ) : null}
            ...
            <div className="grid gap-2">
              <Label htmlFor="teamTagline">Tagline (optional)</Label>
              <Input
                id="teamTagline"
                value={tagline}
            ...
            <div className="grid gap-2">
              <Label htmlFor="teamPhoto">
                {isEditMode ? "Replace photo (optional)" : "Photo"}
              </Label>
              <Input
                id="teamPhoto"
                ref={fileRef}
              ...
              <p className="text-xs text-muted-foreground">
                Stored on the API under /uploads/team/
              </p>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="photoTitle">Image title (SEO)</Label>
              <Input
                id="photoTitle"
                value={photoTitle}
            ...
            <div className="grid gap-2">
              <Label htmlFor="photoAlt">Image description / alt (SEO)</Label>
              <Input
                id="photoAlt"
                value={photoAlt}
              ...
              <TableBody>
                {members.map((member) => (
                  <TableRow
                    key={member.id}
                    className={
                      editing?.id === member.id ? "bg-primary/5" : undefined
                    }
                  >
                    <TableCell>
